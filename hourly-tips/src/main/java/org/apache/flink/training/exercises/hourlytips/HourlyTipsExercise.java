...
public class HourlyTipsExercise {
    ...
    public JobExecutionResult execute() throws Exception {
        ...
        DataStream<Tuple3<Long, Long, Float>> hourlyTips = fares.keyBy(fare -> fare.driverId).window(TumblingEventTimeWindows.of(Time.hours(1))).process(new SumTips());
    ...
    public static class SumTips extends ProcessWindowFunction<TaxiFare, Tuple3<Long, Long, Float>, Long, TimeWindow> {
        ...
        public void process(Long key,
                ProcessWindowFunction<TaxiFare, Tuple3<Long, Long, Float>, Long, TimeWindow>.Context ctx,
                Iterable<TaxiFare> events, Collector<Tuple3<Long, Long, Float>> out) throws Exception {
            float sum = 0;
            for (TaxiFare event: events) {
                sum += event.tip;
            }
            out.collect(new Tuple3<>(ctx.window().getEnd(), key, sum));
        }
    }
}
